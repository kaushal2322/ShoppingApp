...
fun CategoriesView(modifier: Modifier = Modifier){
    val categolist  = remember {
        mutableStateOf<List<catermodel>>(emptyList())
    }
            ...
            .get().addOnCompleteListener(){
                if(it.isSuccessful){
                    val resultlis = it.result.documents.mapNotNull { doc ->
                        doc.toObject(catermodel::class.java)
                    }
                    categolist.value = resultlis
                }
            }
    }
    LazyRow (
...
@Composable
fun CategoryItem(category: catermodel){
    Card (
        modifier = Modifier.size(90.dp).clickable{
GlobalNavigation.navContent.navigate("CatoryProduct/"+category.id)
        },
